...
const jobsContentHtml = (job, id) => {
    ...
    job.Sills.split(",").forEach(tag => {
        let tagBtn = `<button
        class="bg-transparent mb-2 mr-1 hover:bg-black text-xs text-black font-semibold  py-2 px-4 border border-black hover:border-transparent rounded-full">
        ${tag}
        </button>`;
    ...
    <h2 class="title  font-boljd">${job.Role}</h2>
    
    <span size="2">${job.locations??' '}</span><br/>
